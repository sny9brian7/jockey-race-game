...
const RACES = [
  ...
  },
  {
    ...
    rivals: [
      { name: "ヤシマソブリン",     style: "差し", adj: 0.11,  odds: 13 },
      { name: "エアダブリン",       style: "先行", adj: 0.22,  odds: 5.6 },
      { name: "スターマン",         style: "差し", adj: 0.08,  odds: 9.8 },
  ...
  },
  {
    ...
    rivals: [
      { name: "ナチュラリズム",       style: "差し", adj: 0.7,  odds: 2.6, immuneKakari: true },
  ...
  },
  {
    ...
    rivals: [
      { name: "スペシャルウィーク", style: "差し", adj: 0.32,  odds: 2.4 },
      { name: "エアグルーヴ",       style: "差し", adj: 0.21,  odds: 4.8 },
      { name: "セイウンスカイ",     style: "逃げ", adj: 0.15,  odds: 7.9 },
  ...
  },
  {
    ...
    rivals: [
      { name: "スペシャルウィーク", style: "差し", adj: 0.9, odds: 2.2, immuneKakari: true, maxVBonus: 0.5, assertive: true },
  ...
  },
  {
    ...
    rivals: [
      { name: "ハーツクライ",       style: "差し",   adj: 0.3,  odds: 11 },
      { name: "ハイアーゲーム",     style: "差し",   adj: 0.15,  odds: 6.8 },
      { name: "コスモバルク",       style: "先行",   adj: 0.15,  odds: 4.2 },
  ...
  },
  {
    ...
    rivals: [
      { name: "サートゥルナーリア", style: "先行", adj: 0.35,  odds: 2.4 },
      { name: "ラッキーライラック", style: "差し", adj: 0.18,  odds: 4.9 },
  ...
  },
  {
    ...
    rivals: [
      ...
      { name: "ジャックドール", style: "先行",   adj: 0.13,   odds: 7.0 },
      { name: "ダノンベルーガ", style: "差し",   adj: 0.15,  odds: 4.9 },
      { name: "シャフリヤール", style: "差し",   adj: 0.45,  odds: 5.9 },
...
function initRace(raceIdx) {
  ...
  for (let i = 0; i < N; i++) {
    ...
    const h = {
      ...
      // maxVBonus: 最高速度への個別の固定加算(既定0)。adjの比率を崩さず末脚だけ底上げしたい時に使う
      maxV: st ? st.maxV + RACE.spdAdj + e.adj * 0.4 + (e.maxVBonus || 0) + rnd(0.05) - 0.03 : 0,
      early: st ? st.early : 0,
...
function updateHorse(h, dt) {
  ...
  // 前に壁がないと行きたがり、急に詰まると引っ掛かる。壁・スリップ・↓で落ち着く
  {
    ...
    // kakariVolMult: 折り合いゲージの上下動の個別倍率(既定1)。プレイヤー馬に設定すると
    // 同じ操作・状況でもゲージが増減しやすくなる(上昇側・減衰側の両方に効く)
    if (h.isPlayer && RACE.player.kakariVolMult) dk *= RACE.player.kakariVolMult;
